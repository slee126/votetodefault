...
void interp_Val(double valslice[], double kprime, double aggregateK[], double Value[], \
        int prod_type, int ia, int  NK, int  Ny, int  Nk, int  G){      
...
//     Needed Inputs 
    double *k_grid, *aggregateK, *prob, *Z, *mass, *surv_prob, *betahat_d, *prod_vec, \
            *par, *b_grid, *b_dec, *b_net, *betahat_nd, *default_decision;
    k_grid = mxGetPr(prhs[0]);
    prob = mxGetPr(prhs[1]);
    Z = mxGetPr(prhs[2]);
    mass = mxGetPr(prhs[3]);
    surv_prob = mxGetPr(prhs[4]);
    betahat_d = mxGetPr(prhs[5]);
    prod_vec = mxGetPr(prhs[6]);
    aggregateK = mxGetPr(prhs[7]);
    par = mxGetPr(prhs[8]);
    b_grid = mxGetPr(prhs[9]);
    b_dec = mxGetPr(prhs[10]);
    b_net = mxGetPr(prhs[11]);
    default_decision = mxGetPr(prhs[12]);
    betahat_nd = mxGetPr(prhs[13]);
            ...
            for (int iK = 0; iK < NK; iK++){
                ...
                for (int iy = 0; iy < Ny; iy++){
                    ...
                    if (dd == 0){
                        ...
                        bigKprime_nd = getKprime_nd(bigK, aggShock, b_grid[ib], iy, betahat_nd, N_reg_nd, NK, aggregateK);
                        bprime = interp_bprime(bigKprime_nd, aggregateK, b_dec, iy, ib, NK, Nb);
                        bilinear_Val(ValueSlice0_o, bigKprime_nd, bprime, aggregateK, b_grid, Value_o, 0, ia, NK, Nb, Ny, Nk, G);   
                        r = (1 - tauk)*(aggShock*alfa*pow(bigK*3, (alfa - 1)) - delta);
                        receipts = working*.33*taup*aggShock*(1 - alfa)*pow(bigK*3, alfa);
                    }
                    else{
                        bond = 0.0;
                        bigKprime_nd = getKprime_d(bigK, aggShock, iy, betahat_d, N_reg_d, NK, aggregateK);
                        interp_Val(ValueSlice0_o, bigKprime_nd, aggregateK, Value_d, 0, ia, NK, Ny, Nk, G);    
...
//-----------------------------------------------------------------------------------------------------------------------------
                    ...
                    if(ib == Nb - 1){
                        ...
                        bigKprime_d = getKprime_d(bigK, aggShock, iy, betahat_d, N_reg_d, NK, aggregateK);
                        interp_Val(ValueSlice0_d, bigKprime_d, aggregateK, Value_d, 0, ia, NK, Ny, Nk, G);        
            ...
            for (int iK = 0; iK < NK; iK++){
                ...
                for (int iy = 0; iy < Ny; iy++){
                    ...
                    if (dd == 0){
                        ...
                        bilinear_Val(ValueSlice0_o, bigKprime_nd, bprime, aggregateK, b_grid, Value_o, 0, ia, NK, Nb, Ny, Nk, G);    
                        bilinear_Val(ValueSlice1_o, bigKprime_nd, bprime, aggregateK, b_grid, Value_o, 1, ia, NK, Nb, Ny, Nk, G);    
                        bilinear_Val(ValueSlice2_o, bigKprime_nd, bprime, aggregateK, b_grid, Value_o, 2, ia, NK, Nb, Ny, Nk, G);    
                        bilinear_Val(ValueSlice3_o, bigKprime_nd, bprime, aggregateK, b_grid, Value_o, 3, ia, NK, Nb, Ny, Nk, G);    
                    }
                    else{
                        bond = 0;
                        bigKprime_d = getKprime_d(bigK, aggShock, iy, betahat_d, N_reg_d, NK, aggregateK);
                        ...
                        interp_Val(ValueSlice0_o, bigKprime_d, aggregateK, Value_d, 0, ia, NK, Ny, Nk, G);    
                        interp_Val(ValueSlice1_o, bigKprime_d, aggregateK, Value_d, 1, ia, NK, Ny, Nk, G);    
                        interp_Val(ValueSlice2_o, bigKprime_d, aggregateK, Value_d, 2, ia, NK, Ny, Nk, G);    
                        interp_Val(ValueSlice3_o, bigKprime_d, aggregateK, Value_d, 3, ia, NK, Ny, Nk, G);    
                    }
...
//-----------------------------------------------------------------------------------------------------------------------------
                    ...
                    if(ib == Nb - 1){
                        ...
                        bigKprime_d = getKprime_d(bigK, aggShock, iy, betahat_d, N_reg_d, NK, aggregateK);
                        interp_Val(ValueSlice0_d, bigKprime_d, aggregateK, Value_d, 0, ia, NK, Ny, Nk, G);        
                        interp_Val(ValueSlice1_d, bigKprime_d, aggregateK, Value_d, 1, ia, NK, Ny, Nk, G);        
                        interp_Val(ValueSlice2_d, bigKprime_d, aggregateK, Value_d, 2, ia, NK, Ny, Nk, G);        
                        interp_Val(ValueSlice3_d, bigKprime_d, aggregateK, Value_d, 3, ia, NK, Ny, Nk, G);        
